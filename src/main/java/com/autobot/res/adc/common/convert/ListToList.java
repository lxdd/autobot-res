...
public class ListToList {
	 ...
	 * @return
	 */
	public static Optional<List<Catalog>> convertCatalogMoListOpt(Optional<List<CatalogBO>> boListOpt) {

		List<Catalog> moList = null;

		if (boListOpt.isPresent()) {

			moList = new ArrayList<>();

			Catalog mo = null;
			for (CatalogBO bo : boListOpt.get()) {
				mo = new Catalog();
				BeanUtils.copyProperties(bo, mo);
				moList.add(mo);
			}
		...
		// 构建返回
		Optional<List<Catalog>> moListOpt = Optional.ofNullable(moList);
