...
public class CatalogController {
	...
	public Result<Object> updateByServeId(
		...
		MenuTreeUtil menuTree = new MenuTreeUtil();
		Optional<List<CatalogBO>> boListOpt = menuTree.menuToList(jsonStr);

		// 3、批量创建改服务方目录
		Optional<List<Catalog>> moListOpt = ListToList.convertCatalogMoListOpt(boListOpt);
		if (moListOpt.isPresent()) {
			catalogService.batchInsert(moListOpt.get());
		}
